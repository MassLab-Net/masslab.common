...
using MassLab.Common.Authorization.Handlers;
using MassLab.Common.Authorization.Requirements;

namespace MassLab.Common.Authorization.Extensions;
...
public static class AuthorizationServiceCollectionExtensions
{
    ...
    /// </summary>
    public static IServiceCollection AddMassLabAuthorization(this IServiceCollection services)
    {
        ...
        services.AddMassLabAuthorizationPolicyProvider();
        return services;
    }

    private static void AddMassLabAuthorizationPolicyProvider(this IServiceCollection services)
    {
        var existing = services.LastOrDefault(d => d.ServiceType == typeof(IAuthorizationPolicyProvider));
        if (existing is null)
        {
            services.AddSingleton<IAuthorizationPolicyProvider, MassLabAuthorizationPolicyProvider>();
            return;
        }

        if (existing.ImplementationType == typeof(MassLabAuthorizationPolicyProvider))
            return;
...
/// </summary>
internal class MassLabAuthorizationPolicyProvider : DefaultAuthorizationPolicyProvider
{
    ...
    public MassLabAuthorizationPolicyProvider(Microsoft.Extensions.Options.IOptions<AuthorizationOptions> options) :
        base(options) { }
