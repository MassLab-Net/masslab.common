...
Add a project reference to `Victor.Common.Grpc`:

```xml
<ProjectReference Include="..\..\common\Victor.Common.Grpc\Victor.Common.Grpc.csproj" />
```
...
```csharp
using Victor.Common.Grpc.ErrorHandling;
using Grpc.Core;
...
- `Victor.Common.Api` (for exception types)
- `Victor.Common.Validation` (for ValidationException)

## Integration with Other Victor Libraries

This library integrates seamlessly with:
- **Victor.Common.Api**: Uses exception types for error handling
- **Victor.Common.Logging.Serilog**: TraceId is automatically enriched in logs
- **Victor.Common.Validation**: ValidationException mapping for invalid arguments
