...
namespace Victor.Common.RateLimiting.Extensions;
...
{
    /// <summary>Default policy name registered by <c>AddVictorRateLimiting</c>.</summary>
    public const string DefaultPolicyName = "victor-default";

    /// <summary>
    /// Registers a fixed-window rate limiter partitioned by authenticated
    /// user id (falling back to remote IP).
    /// </summary>
    public static IServiceCollection AddVictorRateLimiting(
    ...
    public static IApplicationBuilder UseVictorRateLimiting(this IApplicationBuilder app)
    {
        app.UseRateLimiter();
        return app;
    }
