# Victor.Common.Swagger

Swagger/OpenAPI registration for Victor services, including API version
documents and an optional JWT bearer authorize button.

## Program.cs

```csharp
using Victor.Common.ApiVersioning.Extensions;
using Victor.Common.Swagger.Extensions;

builder.Services.AddVictorApiVersioning();
builder.Services.AddSwaggerWithJwt(builder.Configuration);

var app = builder.Build();
app.UseSwaggerWithUI();
```

## Configuration

```json
{
  "Swagger": {
    "Title": "Product API",
    "Description": "Product service endpoints",
    "EnableJwtBearer": true,
    "RoutePrefix": "swagger"
  }
}
```

Open `/swagger` to inspect API docs. When API versioning is enabled, Swagger UI
shows one document per discovered API version.
