...
using Victor.Common.Swagger.Configuration;

namespace Victor.Common.Swagger.Extensions;
...
public static class SwaggerExtensions
{
    ...
    /// <c>AddVictorApiVersioning()</c> has been called).
    /// </summary>
        ...
        params Assembly[] xmlAssemblies)
    {
        ...
        services.AddOptions<Swashbuckle.AspNetCore.SwaggerGen.SwaggerGenOptions>()
            .Configure<IServiceProvider, IOptions<SwaggerOptions>>((opts, sp, victorOpts) =>
            {
                var provider = sp.GetService<IApiVersionDescriptionProvider>();
                var so = victorOpts.Value;
                if (provider is null)
                {
    ...
    /// version (requires <c>AddVictorApiVersioning()</c>).
    /// </summary>
