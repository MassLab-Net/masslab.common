...
Common logging abstractions and configuration for Victor microservices.

## Overview

This library provides a consistent logging interface across all microservices in the Victor system. It wraps `ILogger<T>` from Microsoft.Extensions.Logging with a custom `ILoggerAdapter<T>` interface that provides a standardized API for logging operations.
...
```xml
<ItemGroup>
  <ProjectReference Include="..\..\common\Victor.Common.Logging\Victor.Common.Logging.csproj" />
</ItemGroup>
```
...
For Serilog integration with enrichers and advanced sinks, use the `Victor.Common.Logging.Serilog` library which builds on top of this library.
...
Internal use only - Victor project.
